...
describe('ProxyAdapter', () => {
  ...
  it('should return the response from the API successfuly', async () => {
    ...
    const request = {
      originalUrl: 'http://example.com/api/endpoint',
      method: 'GET',
      body: {},
      headers: {}
    }
  ...
  it('should return the response from the API with throw error', async () => {
    jest.spyOn(httpService, 'request').mockImplementationOnce(new Error('Async Error') as never)
    ...
    await expect(proxyAdapter.execute(request)).rejects.toThrow('Error occurred while proxying request.')
  })
})
